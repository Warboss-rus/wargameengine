...
void RegisterUI(IScriptHandler& handler, ui::IUIElement* uiRoot, view::TranslationManager& transMan, ui::IUITextHelper& textHelper)
{
	...
	handler.RegisterMethod(CLASS_UI, APPLY_THEME, [&, uiRoot](void* instance, IArguments const& args) {
		...
		std::shared_ptr<UITheme> theme = std::make_shared<UITheme>();
		theme->Load(file);
		c->SetTheme(theme);
		return nullptr;
	});
	...
	handler.RegisterProperty(CLASS_UI, PROPERTY_VISIBLE, [&, uiRoot](void* instance, IArguments const& args) {
		...
		IUIElement * c = instance ? reinterpret_cast<IUIElement*>(instance) : uiRoot;
		c->SetVisible(args.GetBool(1)); }, [&, uiRoot](void* instance) {
		IUIElement * c = instance ? reinterpret_cast<IUIElement*>(instance) : uiRoot;
		return c->GetVisible(); });
	...
	handler.RegisterProperty(CLASS_UI, PROPERTY_TEXT, [&, uiRoot](void* instance, IArguments const& args) {
		...
		IUIElement * c = instance ? reinterpret_cast<IUIElement*>(instance) : uiRoot;
		c->SetText(transMan.GetTranslation(args.GetWStr(1))); }, [&, uiRoot](void* instance) {
		IUIElement * c = instance ? reinterpret_cast<IUIElement*>(instance) : uiRoot;
		return c->GetText(); });
