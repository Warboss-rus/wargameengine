...
public:
	...
	{}
	CImage(unsigned char* data, size_t width, size_t height, unsigned short bpp, int flags = 0, size_t size = 0)
		:m_width(width), m_height(height), m_bpp(bpp), m_data(data), m_flags(flags), m_size(size)
	{}
	CImage(std::vector<unsigned char> && uncompressedData, size_t width, size_t height, unsigned short bpp, int flags = 0, size_t size = 0)
