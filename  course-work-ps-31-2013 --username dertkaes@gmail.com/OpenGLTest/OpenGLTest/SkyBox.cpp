...
void CSkyBox::Draw()
{
	...
	glBegin(GL_QUADS);
		...
		glTexCoord2f(1.0f, 1.0f); glVertex3f(m_x,		  m_y,		m_z + m_length);
		glTexCoord2f(0.0f, 1.0f); glVertex3f(m_x + m_width, m_y,		m_z + m_length); 
		glTexCoord2f(0.0f, 0.0f); glVertex3f(m_x + m_width, m_y,		m_z);	
	glEnd();
	// Back side
	texManager->SetTexture(m_imageFolder + "/back.bmp");
	glBegin(GL_QUADS);
		glTexCoord2f(0.0f, 0.0f); glVertex3f(m_x, m_y + m_height, m_z);
		glTexCoord2f(0.0f, 1.0f); glVertex3f(m_x,		  m_y + m_height,		m_z + m_length);
		glTexCoord2f(1.0f, 1.0f); glVertex3f(m_x + m_width, m_y + m_height,		m_z + m_length); 
		glTexCoord2f(1.0f, 0.0f); glVertex3f(m_x + m_width, m_y + m_height,		m_z);	
	glEnd();
	// Left side
	...
	glBegin(GL_QUADS);
		...
		glTexCoord2f(1.0f, 1.0f); glVertex3f(m_x + m_width, m_y,		m_z + m_length);
		glTexCoord2f(0.0f, 1.0f); glVertex3f(m_x + m_width, m_y + m_height,	m_z + m_length); 
		glTexCoord2f(0.0f, 0.0f); glVertex3f(m_x + m_width, m_y + m_height,	m_z);
	glEnd();
