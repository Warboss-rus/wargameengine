...
CSkyBox::CSkyBox(double width, double height, double length, std::string const& imageFolder): m_width(width), m_height(height), m_length(length)
{
	m_list = glGenLists(1);
	glNewList(m_list, GL_COMPILE);
	CTextureManager * texManager = CTextureManager::GetInstance();
	// Top side
	texManager->SetTexture(imageFolder + "/top.bmp");
	...
	glEnd();
	// Bottom side
	texManager->SetTexture(imageFolder + "/bottom.bmp");
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE_EXT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE_EXT);
	glBegin(GL_QUADS);
	...
	glEnd();
	// Front side
	texManager->SetTexture(imageFolder + "/front.bmp");
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE_EXT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE_EXT);
	glBegin(GL_QUADS);
	glTexCoord2f(1.0f, 0.0f); glVertex3d(0.0, 0.0, 0.0);
	...
	glEnd();
	// Back side
	texManager->SetTexture(imageFolder + "/back.bmp");
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE_EXT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE_EXT);
	glBegin(GL_QUADS);
	glTexCoord2f(0.0f, 0.0f); glVertex3d(0.0, m_height, 0.0);
	...
	glEnd();
	// Left side
	texManager->SetTexture(imageFolder + "/left.bmp");
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE_EXT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE_EXT);
	glBegin(GL_QUADS);
	...
	glEnd();
	// Right side
	texManager->SetTexture(imageFolder + "/right.bmp");
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE_EXT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE_EXT);
	glBegin(GL_QUADS);
